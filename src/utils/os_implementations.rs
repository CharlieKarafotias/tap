...
pub(super) fn derive_data_local_dir_by_os(
    ...
    match OS {
        ...
        "windows" => {
            if let Ok(local_app_data) = var("LOCALAPPDATA") {
                return Ok(PathBuf::from(format!(
...
pub trait LinkOpener {
    fn open(&self, link: &str) -> Result<(), OsImplementationError> {
        let mut cmd =
            match OS {
                "macos" => {
                    Command::new("open")
                ...
                "linux" => Command::new("xdg-open").arg(link).spawn().map_err(|e| {
...
pub struct RealLinkOpener;
impl LinkOpener for RealLinkOpener {}
...
pub struct OsImplementationError {
    pub kind: OsImplementationErrorKind,
    pub message: String,
}
